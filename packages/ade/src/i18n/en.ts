import type { Messages } from "./it"
import { voiceEn } from "./voice-en"

/**
 * ADE's texts in English.
 *
 * Written as product copy, not as a gloss of the Italian: the same meaning,
 * in the words an English-language app would use for it.
 */
export const en: Messages = {
  ...voiceEn,
  "settings.language.label": "Language",
  "settings.language.title": "Language",
  "settings.language.desc":
    "The language of ADE's menus, panels and notices. System follows your computer's language. The voice assistant keeps using the speech-recognition language, which you set in the voice settings.",
  "settings.language.group": "Interface language",
  "settings.language.system": "System",
  "settings.language.systemNow": (language) => `System (${language})`,
  "settings.language.it": "Italiano",
  "settings.language.en": "English",

  // Command palette
  "palette.desktopOnly": "Needs the desktop app",
  "palette.group.session": "Session",
  "palette.group.project": "Project",
  "palette.group.pane": "Panel",
  "palette.group.view": "View",
  "palette.group.process": "Process",
  "palette.group.recent": "Recent projects",
  "palette.session.new": "New session",
  "palette.project.open": "Open project",
  "palette.pane.close": "Close panel",
  "palette.pane.shrink": "Restore panel size",
  "palette.pane.expand": "Maximize panel",
  "palette.pane.rename": "Rename session",
  "palette.noFocusedPane": "No panel is focused",
  "palette.view.next": (view) => `Next section (${view})`,
  "palette.view.goTo": (view) => `Go to ${view}`,
  "palette.view.here": "You're already here",
  "palette.theme.toggle": "Toggle theme",
  "palette.voice.on": "Turn on voice control",
  "palette.voice.off": "Turn off voice control",
  "palette.voice.unsupported": "This browser doesn't support speech recognition",
  "palette.voice.settings": "Voice settings",
  "palette.record.start": "Record the window",
  "palette.record.stop": "Stop recording",
  "palette.record.desktopOnly": "Recording only works in the desktop app",
  "palette.record.qualityIs": (quality) => `Video quality: ${quality}`,
  "palette.record.quality": "Recording video quality",
  "palette.record.export": "Export the last recording with zoom and clicks",
  "palette.record.micOn": "Turn on the microphone for recordings",
  "palette.record.micOff": "Turn off the microphone for recordings",
  "palette.record.folder": "Recordings folder",
  "palette.browser.new": "Open browser",
  "palette.video.new": "Open video",
  "palette.model.new": "Open 3D model",
  "palette.app.new": "Open app simulator",
  "palette.decisions.open": "Decisions for you",
  "palette.decisions.pane": "Open Decisions panel",
  "palette.update.check": "Check for updates",
  "palette.process.kill": "Kill process",
  "palette.process.none": "The focused panel has no running process",

  // Updates, presence, pane states, presets, new-pane menu, restore
  "update.skip.hidden": "The window isn't in the foreground.",
  "update.skip.recent": "Checked a moment ago.",
  "update.skip.budget": "Too many checks this hour. Trying again later.",
  "update.skip.other": "Check skipped.",
  "update.available": (version) => `ADE ${version} is available`,
  "update.current": (version) => `You're up to date: ADE ${version} is the latest version.`,
  "update.none": "No updates available.",
  "update.dev": "This is a development build, so there's no installed version to update.",
  "update.failed": (problem) => `Couldn't check for updates: ${problem}`,
  "presence.off": "Microphone off",
  "presence.asleep": "Waiting for the wake word",
  "presence.idle": "Ready",
  "presence.listening": "Listening…",
  "presence.confirming": "Waiting for confirmation",
  "presence.dictating": "Dictating",
  "presence.executing": "Working on it",
  "paneState.work": "Working",
  "paneState.perm": "Waiting for permission",
  "paneState.ask": "Waiting on another session",
  "paneState.err": "Stuck",
  "paneState.limit": "Limit reached",
  "paneState.idle": "Ready",
  "paneState.short.perm": "Permission",
  "paneState.short.ask": "Waiting",
  "paneState.short.limit": "Limit",
  "preset.solo": "Solo",
  "preset.solo.desc": "One agent in a terminal.",
  "preset.pair": "Pair",
  "preset.pair.desc": "One builds, one reviews the same tree.",
  "preset.workbench": "Workbench",
  "preset.workbench.desc": "An agent plus a shell for git and tests.",
  "preset.swarm": "Swarm",
  "preset.swarm.desc": "Four agents split up parallel tasks.",
  "hooks.unavailable": "not available in this build",
  "stats.who": (processes) => `ADE and its ${processes} processes (webview and agents)`,
  "stats.cpu": (who, percent) => `CPU used by ${who}: ${percent}%`,
  "stats.ram": (who, size) => `Memory used by ${who}: ${size}`,
  "stats.mem": (total) => `Share of this computer's memory (${total}) used by ADE`,
  "paneTitle.agent": "Session",
  "paneTitle.reviewer": "Review",
  "paneTitle.shell": "Terminal",
  "newPane.session": "Session",
  "newPane.session.hint": "an agent in a terminal",
  "newPane.browser.hint": "the page you're building",
  "newPane.video.hint": "watch a project file, and show it to the agent",
  "pane.video.title": "Video",
  "newPane.model": "3D model",
  "newPane.model.hint": "glTF, OBJ, STL or FBX, reloaded when the file changes",
  "newPane.app": "App simulator",
  "newPane.app.hint": "your dev server's app in a phone or a window",
  "newPane.decisions": "Decisions",
  "newPane.decisions.hint": "the whole log: open, answered, deferred, closed",
  "restore.note": "Session restored.",
  "restore.note.gone": "The process is no longer running.",
  "restore.note.reopen": "Reopening the agent's conversation where it left off.",
  "restore.note.rerun": "The process didn't survive the shutdown, so the task starts again.",

  // Sidebar
  "sidebar.active": "active",
  "sidebar.project": "project",
  "sidebar.elapsed": "Time elapsed",
  "sidebar.spaces": "Spaces",
  "sidebar.addSpace": "Add a space",
  "sidebar.addRemote": "Add a remote environment (ssh)",
  "sidebar.noSpaces": "No spaces open.",
  "sidebar.agents": "Active agents",
  "sidebar.newAgentSession": "New agent session",
  "sidebar.noAgents": "No active agents.",
  "sidebar.startAgent": "+ Start a new agent",
  "sidebar.files": "Files",
  "sidebar.files.noProject": "Open a project to see its files.",
  "sidebar.search.project": "Search the project…",
  "sidebar.search.open": "Search open files…",
  "sidebar.search.help": "Several words must all match. A word with / searches the path (e.g. grid/pane).",
  "sidebar.search.clear": "Clear search",
  "sidebar.search.show": "Show",
  "sidebar.search.files": "Files",
  "sidebar.search.folders": "Folders",
  "sidebar.search.searching": "Searching the project…",
  "sidebar.search.noMatch": "No files or folders match.",
  "sidebar.search.noFile": "No files match.",
  "sidebar.search.noFolder": "No folders match.",
  "sidebar.settings": "Settings",
  "sidebar.stats": "Resources used by ADE",
  "sidebar.resize": "Resize sidebar",

  // Panels, grid, palette, empty project
  "pane.tree.stale.short": "last commit only",
  "pane.tree.noDeps.short": "no dependencies",
  "pane.tree.full": "Isolated worktree with your current work",
  "pane.tree.stale": "Isolated worktree from the last commit, without your uncommitted changes",
  "pane.tree.noDeps": "Isolated worktree without linked dependencies: the agent can edit but not run",
  "pane.tree.project": "No isolated worktree: the agent works directly in the project",
  "pane.tree.preparing": "Preparing worktree…",
  "pane.tree.preparing.short": "preparing worktree…",
  "pane.session": "Session",
  "pane.renameTip": "Double-click to rename",
  "pane.name": "Session name",
  "pane.quota": "quota",
  "pane.quota.na": "n/a",
  "pane.toBottom": "Jump to bottom",
  "pane.input": "Message the agent…",
  "pane.input.none": "no process listening",
  "pane.notDelivered": (text) => `No process is listening, so ${text} wasn't delivered.`,
  "pane.pluginGone": "The plugin is no longer loaded.",
  "grid.resize": "Resize panel",
  "grid.resize.tip": "Drag to resize · double-click for the default size",
  "palette.label": "Command palette",
  "palette.search": "Search commands…",
  "palette.empty": "No commands found.",
  "preview.binary": "Binary file or unsupported encoding.",
  "preview.loading": "Loading…",
  "empty.title": "No project open",
  "empty.browser": "In the browser, ADE can't read the disk or start processes. Worktrees and real sessions are only available in the desktop app.",
  "empty.desktop": "Choose a folder to get started. In a git repository, each session gets its own isolated working copy; otherwise agents write straight into the folder.",
  "empty.open": "Open project",
  "projectBar.noGit": "Not a git repository: agents edit the project files directly.",
  "projectBar.noGit.short": "not isolated",

  // New session, session resume hooks, Agent console
  "new.title": "New session",
  "new.cancel": "Cancel",
  "new.close": "Close",
  "new.launch": (count) => count === 1 ? "Start 1 session" : `Start ${count} sessions`,
  "new.agent": "Agent",
  "new.notInstalled": "not installed",
  "new.missing": "missing",
  "new.count": "How many",
  "new.count.label": "parallel sessions",
  "new.preview": "Will start",
  "new.role.reviewer": "reviews",
  "new.summary": (count, agent, workspace) => `${count} ${count === 1 ? "session" : "sessions"} with ${agent} in ${workspace}`,
  "hooks.title": "Session resume",
  "hooks.desc": "When ADE restarts, it reopens your sessions where they left off. To do that it needs to know which conversation each panel had. Some CLIs accept an ID chosen by ADE; others, like codex, don't, and only they can report it. Here ADE adds an entry to that CLI's configuration so it reports the ID whenever a session starts.",
  "hooks.files": "These files don't belong to ADE. They're shown in full below, any entries already in them are left alone, and Remove puts the configuration back the way it was.",
  "hooks.state.unavailable": "unavailable",
  "hooks.state.on": "on",
  "hooks.state.broken": "needs reinstalling",
  "hooks.state.off": "off",
  "hooks.broken": "The configuration has an ADE entry that doesn't match the script on disk, so the CLI is running a hook that does nothing. Reinstall to fix it.",
  "hooks.install": "Install",
  "hooks.reinstall": "Reinstall",
  "hooks.remove": "Remove",
  "hooks.outside": "The script does nothing outside ADE: it exits at the first environment variable it can't find, so the same CLI started from any other terminal behaves exactly as before.",
  "agent.mic.start": "Turn on microphone",
  "agent.mic.stop": "Turn off microphone",
  "agent.settings": "Settings",
  "agent.noPlanner": "Without an OpenRouter key in the voice settings, the assistant only understands fixed commands. It can't plan requests like “start four claude sessions”.",
  "agent.empty.title": "Nothing here yet.",
  "agent.empty.body": "Talk to the assistant or type below. What you ask and everything it does for you shows up here.",
  "agent.empty.example1": "«start four claude sessions on this project»",
  "agent.empty.example2": "«open panel two»",
  "agent.empty.example3": "«what is happening»",
  "agent.input": "Message the assistant…",
  "agent.send": "Send",
  "agent.sendHeld": "Send this",
  "agent.held": (text) => `Heard while I was thinking: “${text}”`,
  "agent.plan": "Plan complete",
  "agent.plan.count": (ok, total) => `${ok} of ${total}`,

  // Workbench: recording, updates, editor, startup, bar, settings
  "record.desktopOnly": "Recording only works in the desktop app.",
  "record.noFolder": "No folder chosen, so recording was cancelled.",
  "record.minimized": "Recording stopped because the ADE window was minimized.",
  "record.nothingToExport": "There's no recording to export in this session.",
  "record.exporting": "Exporting the video with zoom and clicks. This takes as long as the recording.",
  "record.mic.on": "Your recordings will include the microphone.",
  "record.mic.off": "Your recordings won't include the microphone.",
  "record.quality.set": (level, size) => `Video quality: ${level} — ${size}.`,
  "record.active.mic": (path) => `Recording with the microphone — ${path}`,
  "record.active.noMic": (path) => `Recording without the microphone — ${path}`,
  "record.closing": "Finishing the file",
  "picker.video": "Choose a video",
  "picker.video.filter": "Videos",
  "picker.model": "Choose a 3D model",
  "picker.model.filter": "3D models",
  "decisions.delivered": (key) => `Decision ${key} delivered from the Decisions panel`,
  "decisions.waiting": "Decisions waiting for you",
  "update.desktopOnly": "Updates are checked from the desktop app.",
  "update.alreadyShown": (version) => `Already in your notifications: ADE ${version} is available.`,
  "update.openFailed": (problem) => `Couldn't open the page: ${problem}`,
  "update.restart": (running) => `ADE will restart to update. ${running === 1 ? "The running session will be stopped and resumed" : `The ${running} running sessions will be stopped and resumed`} when it reopens.`,
  "update.restart.title": "Update ADE",
  "update.restart.ok": "Update and restart",
  "update.restart.later": "Later",
  "update.installFailed": (problem) => `The update failed (${problem}). Opening the release page.`,
  "update.checking": "Checking…",
  "voice.noMic": "No microphone is available here.",
  "voice.noHost.download": "Can't download the voice here: it needs the desktop app.",
  "voice.noHost": "Voice needs the desktop app.",
  "plugins.consent.title": "Project plugins",
  "plugins.consent.run": "Run",
  "plugins.consent.later": "Not now",
  "boot.host": "connecting to ADE",
  "boot.restore": "restoring sessions",
  "boot.project": "opening the project",
  "editor.closeDirty": (path) => `${path}\n\nYou have unsaved changes. Closing will discard them.\n\nClose anyway?`,
  "editor.saveUnreadable": (path, problem) => `${path}\n\nCouldn't re-read the file to check whether it changed (${problem}).\n\nSave anyway and replace what's on disk?`,
  "editor.saveCancelled.unreadable": (problem) => `Save cancelled: couldn't re-read the file (${problem}).`,
  "editor.saveChanged": (path) => `${path}\n\nThe file changed on disk since you opened it. Saving will replace those changes with yours.\n\nContinue?`,
  "editor.saveCancelled.changed": "Save cancelled: the file changed on disk.",
  "keys.unread": (problem) => `Couldn't read the API keys, so the session starts without them: ${problem}`,
  "task.notSent": "ADE didn't send the initial task because the session never settled. Type it yourself once it's ready.",
  "task.notSent.short": "Initial task not sent: the session wasn't ready.",
  "task.stepNotSent": (text) => `ADE didn't type “${text}” because the connection never settled. Type it yourself.`,
  "task.none": "No initial task",
  "bar.newPane": "New panel",
  "bar.windowControls": "Window controls",
  "bar.dismissNotice": "Dismiss notice",
  "bell.empty": "No notifications.",
  "settings.title": "Settings",
  "settings.subtitle": "Voice, routines, code, MCP, plugins and skills",
  "settings.group.voice": "Voice",
  "settings.routine": "Routines",
  "settings.routine.desc": "Things ADE does on its own: at a set time, when a project opens, or when a session ends.",
  "settings.routine.instead": "For now, sessions are started by hand from the launch screen.",
  "settings.code": "Code",
  "settings.keys": "API keys",
  "settings.extensions": "Extensions",
  "settings.tools": "Tools",
  "settings.noPlugins": "No plugins loaded.",
  "settings.notBuilt": "Nothing to configure here yet: the section exists, the feature doesn't.",
  "settings.grid.title": "Grid",
  "settings.grid.desc": "How many columns the panels use in the Code view. On Auto, ADE picks based on the window width and how many panels are open, so none gets narrower than is readable.",
  "settings.grid.columns": "Grid columns",
  "settings.grid.auto": "Auto",
  "settings.mcp.desc": "The servers ADE connects to over the Model Context Protocol, and the tools they provide.",
  "settings.mcp.instead": "In the meantime, the agent CLIs ADE starts use their own MCP configuration, the same one they'd use from a terminal.",
  "settings.bots.title": "Bots",
  "settings.bots.desc": "The nikcli agents known to this machine: which model they run with, and whether they belong to the project or are global. Create and edit them in the Bot view.",
  "settings.bots.empty": "No nikcli agents yet. Create one in the Bot view.",
  "settings.bots.reading": "Reading nikcli folders…",
  "settings.bots.defaultModel": "nikcli model",
  "settings.bots.scopeProject": "project",
  "settings.bots.scopeGlobal": "global",
  "settings.skills.title": "Tools",
  "settings.skills.desc": "Which tools have been disabled for a bot. Any bot not listed here has all of them: nikcli only records disabled tools in the file.",
  "settings.skills.empty": "No bot has restrictions: all bots can use every nikcli tool.",
  "settings.skills.without": (tools) => `without ${tools}`,
  "settings.providers.title": "Providers",
  "settings.providers.desc1": "The runners a bot can use, each with its own CLI account: Anthropic subscriptions go through Claude Code, ChatGPT through Codex, and keys and other subscriptions through nikcli. The engine, model, and reasoning effort are set in each bot's card.",
  "settings.providers.desc2Before": (max) => `ADE never asks for or reads credentials: sign-in happens in each CLI's official flow. Subscriptions are for personal use and ADE keeps at most ${max} turns at once for each; for intensive or unattended automation sign in to the CLI with an API key (Claude Code accepts the Anthropic Console key, Codex the OpenAI key with `,
  "settings.providers.desc2After": ").",
  "settings.providers.checking": "Checking…",
  "settings.providers.notInstalled": "Not installed",
  "settings.providers.connected": "Connected",
  "settings.providers.notConnected": "Not connected",
  "settings.providers.unverified": "To verify",
  "settings.providers.switchAccount": "Switch account",
  "settings.providers.login": "Sign in",
  "settings.providers.checkAgain": "Check again",

  // Splash & logo
  "splash.aria": "ADE onboarding — 3D ASCII typography",
  "logo.title": "ADE — liquid chrome mercury with physics (click to splash)",
  "logo.aria": "ADE Logo — molten chrome",

  // Voice shortcut & permission
  "voice.shortcut.feature.agent": "voice assistant",
  "voice.shortcut.feature.transcription": "dictation",
  "voice.shortcut.busy": (chord, feature) => `The shortcut ${chord} for ${feature} is unavailable: another application might be using it. Choose another one in voice settings.`,
  "voice.permission.notRefusal": "None of the proposed answers is a refusal: please answer yourself, I won't choose for you.",

  // Bar, window, bell, panel actions
  "pane.expand": "Maximize",
  "pane.close": "Close",
  "bar.sessions": (count) => `${count} ${count === 1 ? "session" : "sessions"}`,
  "bar.sections": "Sections",
  "bar.palette": "Search or run",
  "window.minimize": "Minimize",
  "window.maximize": "Maximize or restore",
  "window.close": "Close",
  "bell.title": "Notifications",
  "bell.dismiss": "Dismiss",

  // Export, stuck note
  "record.export.busy": "An export is already running.",
  "record.export.desktopOnly": "Exporting only works in the desktop app.",
  "record.export.done": (path) => `Video ready: ${path}`,
  "record.export.failed": (problem) => `Export failed: ${problem}`,
  "pane.maybeStuck": (request) => `Possibly stuck: working for over an hour with no output or changes (request ${request})`,

  // Pane activity (labels for the codes in grid/activity.ts)
  "activity.ready": "Ready",
  "activity.running": "Running",
  "activity.starting": "Starting",
  "activity.done": "Done",
  "activity.exited": (code) => `Exited with ${code}`,
  "activity.resumed": "Session resumed",
  "activity.toResume": "To resume",
  "activity.restored": "Restored",
  "activity.killed": "Killed",
  "activity.permission": "Waiting for permission",
  "activity.startFailed": "Failed to start",
  "activity.sshConnecting": "Connecting over ssh",
  "activity.connected": "Connected",
  "activity.connectFailed": "Connection failed",
  "pane.killed": "Process killed",
  "pane.limit": "limit reached",
  "pane.limit.window": (window) => `${window} window used up`,

  // Browser panel
  "browser.unreachable": "Can't reach the server",
  "browser.fidelity.native": "Native (bridge on)",
  "browser.fidelity.mirror": "Mirror (isolated copy)",
  "browser.fidelity.none": "Read-only",
  "browser.fidelity.pending": "Connecting…",
  "browser.fluid": "Fluid",
  "browser.grip": "Drag to move the panel",
  "browser.back": "Back",
  "browser.forward": "Forward",
  "browser.reload": "Reload",
  "browser.mode.browse": "Browse",
  "browser.mode.browse.tip": "Browse mode",
  "browser.mode.edit": "Inspect",
  "browser.mode.edit.tip": "Inspect and edit mode",
  "browser.rotate": "Rotate",
  "browser.preview": "Browser preview",
  "browser.error.title": "Couldn't load this URL",
  "browser.error.hint": "Make sure the server is running and reachable.",
  "browser.retry": "Try again",
  "browser.blocked.title": "This site can't be shown inside ADE",
  "browser.blocked.msg": "The server forbids embedding the page in another app (X-Frame-Options or CSP frame-ancestors) and doesn't let ADE read a copy of it.",
  "browser.openExternal": "Open in browser",
  "browser.openExternal.failed": (problem) => `Couldn't open it in the browser: ${problem}`,
  "browser.noCopy.title": "Inspect isn't available for this page",
  "browser.noCopy.msg": "To select elements ADE needs a copy of the page, and this site doesn't allow reading one (CORS). In Browse the page stays visible.",
  "browser.noCopy.back": "Back to Browse",
  "browser.context": "Captured context:",
  "browser.clearSelection": "Clear selection",
  "browser.prompt.selected": "Describe what to change…",
  "browser.prompt.empty": "Point at an element on the page, or type an instruction…",
  "browser.selection.none": "No elements selected",
  "browser.selection.count": (count) => count === 1 ? "1 element selected" : `${count} elements selected`,
  "browser.owner.none": "Not linked",
  "browser.owner.ready": (title) => `↳ ${title}`,
  "browser.owner.closed": (title) => `${title} (closed)`,
  "browser.owner.tip": "The session that gets the inspector's requests",
  "browser.owner.focus": "Go to session",
  "browser.owner.bind": "Link to",
  "browser.owner.unbind": "Unlink",
  "browser.send.ask": "Which session should get this request? The pane will stay linked to it.",
  "browser.send.closed": (title) => `“${title}” is not running. Which session should get this request?`,
  "browser.send.none": "No session is running in this project.",
  "browser.send.sending": "Sending…",
  "browser.send.sent": (title) => `Sent to “${title}”: it arrives when its turn ends.`,
  "browser.send.failed": (reason) => `Not sent: ${reason}`,
  "browser.send.stopped": "the session is no longer running",
  "browser.send.noProject": "no open project to save the details in",
  "browser.shot.unavailable": "this version of ADE takes no screenshots",
  "browser.section.hint": "Alt+click: whole section",
  "browser.section.whole": (name) => `All of ${name}`,
  "browser.section.whole.tip": (label) => `Select the whole section: ${label}`,
  "browser.edit.toggle": "Edit",
  "browser.edit.text": "Text",
  "browser.edit.color": "Color",
  "browser.edit.background": "Background",
  "browser.edit.fontSize": "Size",
  "browser.edit.padding": "Padding",
  "browser.edit.radius": "Radius",
  "devServer.region": "Dev servers started",
  "devServer.found": (title, url) => `“${title}” started a server at ${url}`,
  "devServer.open": "Open in a web pane",
  "devServer.dismiss": "Dismiss",
  "note.browserRequest": (title) => `Request from the web pane “${title}” queued: it arrives when the turn ends.`,

  // Decisions
  "date.today": "today",
  "date.tomorrow": "tomorrow",
  "date.monday": "Monday",
  "date.inDays": (days) => `in ${days} days`,
  "decisions.title": "Decisions",
  "decisions.openCount": (count) => `${count} open`,
  "decisions.count": (count) => count === 1 ? "1 decision" : `${count} decisions`,
  "decisions.needAnswer": "choose an option or write your answer",
  "decisions.from": (who) => `from ${who}`,
  "decisions.unlocks": (what) => `unblocks: ${what}`,
  "decisions.options": "Options",
  "decisions.note": "Note",
  "decisions.note.optional": "Note (optional)",
  "decisions.answer": "Answer",
  "decisions.answer.yours": "Your answer",
  "decisions.defer": "Defer",
  "decisions.defer.open": "Defer…",
  "decisions.defer.until": "Defer until",
  "decisions.defer.back": "Bring it back:",
  "decisions.defer.date": "Date",
  "decisions.submit": "Record",
  "decisions.submitNext": "Record and next",
  "decisions.noProject": "No project open.",
  "decisions.noProject.hint": "The decision log lives in the project's .ade/decisions.jsonl.",
  "decisions.unreadable": (problem) => `Can't read the log: ${problem}`,
  "decisions.ignored": (count) => `${count === 1 ? "1 line" : `${count} lines`} skipped in the log`,
  "decisions.section.open": "To decide, in order",
  "decisions.section.answered": "Answered, waiting to be carried out",
  "decisions.section.later": "Coming up",
  "decisions.section.closed": (count) => `Closed · ${count}`,
  "decisions.none": "No open decisions.",
  "decisions.pill.open": "open",
  "decisions.pill.answered": "answered",
  "decisions.pill.deferred": (day) => `deferred · ${day}`,
  "decisions.pill.closed": (day) => `closed ${day}`,
  "decisions.change": "Change answer",
  "decisions.reopen": "Reopen now",
  "decisions.delivery.done": (to, when) => `✓ delivered to ${to} · ${when} · whoever carries it out closes it`,
  "decisions.delivery.by": (who, day) => `answered by ${who} · ${day}`,
  "decisions.queued.ready": (title) => `queued: goes out as soon as “${title}” is free`,
  "decisions.queued.idle": (title) => `queued: goes out once “${title}” is running`,
  "decisions.queued.none": "queued: no session chosen to receive answers",
  "decisions.recipient": "Send answers to",
  "decisions.recipient.nobody": "no session",
  "decisions.recipient.stopped": "(stopped)",
  "decisions.recipient.closed": "(closed)",
  "decisions.recipient.confirm": (queued, title) => `Deliver ${queued === 1 ? "the queued answer" : `the ${queued} queued answers`} to “${title}”?`,
  "decisions.recipient.deliver": "Deliver",
  "decisions.recipient.none": "No session receives answers, so they stay queued until you choose one.",
  "decisions.recipient.idle": (title) => `“${title}” isn't running, so answers stay queued.`,
  "decisions.recipient.waiting": (count) => `${count} waiting.`,
  "decisions.sheet.position": (index, total) => `${index} of ${total}`,
  "decisions.sheet.empty": "When a session opens one, the badge at the top shows it.",
  "decisions.sheet.needChoice": "Nothing chosen: press 1–9 or click an option, then Enter",
  "decisions.sheet.needText": "Write your answer in the note, then press Ctrl+Enter",
  "decisions.sheet.keys": "1–9 choose · Enter records · ← → browse · Esc closes",
  "decisions.sheet.queued.none": (count) => `${count === 1 ? "1 answer" : `${count} answers`} queued: no session receives them`,
  "decisions.sheet.queued.idle": (count) => `${count === 1 ? "1 answer" : `${count} answers`} queued: the chosen session isn't running`,
  "decisions.sheet.full": "Full view",
  "decisions.hint.ready": (title) => `→ ${title}, as a message`,
  "decisions.hint.idle": (title) => `→ queued: “${title}” isn't running`,
  "decisions.hint.none": "→ queued: choose who gets answers in the full view",

  // Editor, review, plugins, screenshots
  "editor.truncatedSave": "The file was only partly loaded, so saving would destroy the part that wasn't read.",
  "editor.nothingToSave": "Nothing to save.",
  "editor.noFile": "No file open.",
  "editor.loading": "Loading file…",
  "editor.truncatedBanner": "This file is too large and was only partly loaded. Saving is turned off to protect the file on disk.",
  "editor.modified": "Modified",
  "editor.saved": "Saved",
  "editor.unsaved": "Unsaved changes",
  "editor.revert": "Revert",
  "editor.revert.tip": "Discard unsaved changes",
  "editor.revert.none": "No changes to revert",
  "editor.save": "Save",
  "editor.save.tip": "Save to disk (Ctrl+S)",
  "editor.label": (path) => `Editor for ${path}`,
  "editor.truncated": "Truncated",
  "editor.cursor": (line, column) => `Line ${line}, Column ${column}`,
  "editor.lines": (count) => `${count} ${count === 1 ? "line" : "lines"}`,
  "plugins.renderFailed": "This plugin couldn't draw its panel.",
  "plugins.sectionFailed": "Section unavailable.",
  "plugins.summary": (active) => `${active} ${active === 1 ? "plugin" : "plugins"} active.`,
  "plugins.summary.failed": (active, failed) => `${active} ${active === 1 ? "plugin" : "plugins"} active, ${failed} failed to load.`,
  "plugins.commands": (count) => `${count} ${count === 1 ? "command" : "commands"}`,
  "plugins.panes": (count) => `${count} ${count === 1 ? "panel" : "panels"}`,
  "plugins.sections": (count) => `${count} ${count === 1 ? "section" : "sections"}`,
  "review.loading": "Loading changes…",
  "review.empty": "No changes to show.",
  "review.header": (added, removed) => `Changes (${added} added, ${removed} removed)`,
  "review.truncated": "The diff is too large and was cut short. Only the file list is shown.",
  "review.renamedFrom": (path) => `(renamed from ${path})`,
  "review.binary": "This is a binary file, so there's no text diff to show.",
  "review.tooLarge": "Content unavailable (diff too large).",
  "review.unparsable": "The changes couldn't be read, or the file is empty.",
  "review.pick": "Select a file to see its changes.",
  "review.error.noGit": "This folder isn't a git repository.",
  "review.error.status": "Couldn't read the repository status.",
  "review.error.stats": "Couldn't compute the change statistics.",
  "review.error.files": "Couldn't list the changed files.",
  "shots.dismiss": "Remove from tray",
  "shots.dismiss.named": (name) => `Remove ${name}`,
  "shots.hint": "drag it onto a session to give it to the agent",
  "shots.delete": "delete",
  "shots.close": "close",

  // Extensions
  "extensions.missingType": "no type \"http\": Claude Code won't load it, other clients will",
  "extensions.filter": "Filter",
  "extensions.filter.all": "All",
  "extensions.filter.oneClick": "One click",
  "extensions.filter.guide": "With a guide",
  "extensions.filter.official": "Official",
  "extensions.transport.remote": "remote",
  "extensions.transport.stdio": "local (stdio)",
  "extensions.after.oauth": "The agent asks you to sign in (OAuth) the first time it's used.",
  "extensions.after.none": "No credentials needed.",
  "extensions.after.oauthOrEnv": (list) => `Sign in with OAuth on first use, or set ${list} in the agent's environment.`,
  "extensions.after.env": (list) => `Set ${list} in the agent's environment. .mcp.json only keeps a reference to it.`,
  "extensions.added": (name, file, hint) => `Added ${name} to ${file}. ${hint} It applies to sessions started from now on.`,
  "extensions.removed": (name, file) => `Removed ${name} from ${file}.`,
  "extensions.tab.installed": (count) => `Installed · ${count}`,
  "extensions.tab.catalog": (count) => `MCP catalog · ${count}`,
  "extensions.tab.plugins": (count) => `Plugins · ${count}`,
  "extensions.desc": (file) => `MCP servers and plugins. Add to project only writes the open project's ${file}, which agent CLIs read when a session starts. Credentials never go in that file.`,
  "extensions.noProject": (file) => `Open a project to add MCP servers. They're installed in its ${file}.`,
  "extensions.search": "Search servers: Stripe, calendar, database…",
  "extensions.search.label": "Search the catalog",
  "extensions.noMatch": "No servers match.",
  "extensions.badge.official": "official",
  "extensions.installed": "✓ In project",
  "extensions.add": "Add to project",
  "extensions.adding": "Adding…",
  "extensions.nameTaken": (name, file) => `“${name}” is already used by another server in ${file}`,
  "extensions.guide": "Setup guide",
  "extensions.guide.tip": "Can't be installed in one click, so this opens the official guide",
  "extensions.source": "Source",
  "extensions.group.servers": "Project MCP servers",
  "extensions.group.plugins": "ADE plugins",
  "extensions.noServers": (file) => `No servers in ${file}. Add one from the catalog.`,
  "extensions.incomplete": "incomplete definition",
  "extensions.variables": (list) => `variables: ${list}`,
  "extensions.removing": "Removing…",

  // API keys
  "keys.problem.noName": "a name is required",
  "keys.problem.nameChars": "letters, digits, spaces, . _ -",
  "keys.problem.noEnv": "a variable name is required",
  "keys.problem.envChars": "capitals, digits and _, e.g. OPENAI_API_KEY",
  "keys.problem.reserved": (env) => `${env} is reserved`,
  "keys.problem.taken": (env) => `${env} already belongs to another key`,
  "keys.problem.noValue": "paste the value",
  "keys.problem.tooLong": "too long (4096 characters at most)",
  "keys.problem.newline": "contains a line break; paste it on one line",
  "keys.billing": (agent, account) => `${agent} will use this key instead of ${account}, which means paid usage`,
  "keys.billing.claude": "your Claude subscription",
  "keys.billing.chatgpt": "your ChatGPT sign-in",
  "keys.added.today": "added today",
  "keys.added.yesterday": "added yesterday",
  "keys.added.days": (days) => `added ${days} days ago`,
  "keys.desc": "Secret keys your sessions can use. The value stays in the system keychain (Windows Credential Manager, macOS Keychain). ADE never writes it to a file or shows it, and only passes it as an environment variable to the agents you choose for each key, when a session starts.",
  "keys.noKeychain": "This version of ADE can't access the keychain.",
  "keys.none": "No saved keys.",
  "keys.hidden": "hidden value",
  "keys.missingValue": "value missing from the keychain",
  "keys.agents": (agents) => `for ${agents}`,
  "keys.agents.none": "no agents, so it isn't passed on",
  "keys.copied": (name, seconds) => `Copied “${name}”. The clipboard clears in ${seconds} seconds`,
  "keys.copy": "Copy",
  "keys.edit": "Edit",
  "keys.delete": "Delete",
  "keys.delete.confirm": "Yes, delete",
  "keys.deleted": (name) => `Deleted “${name}” from the keychain`,
  "keys.updated": (name) => `Updated “${name}”`,
  "keys.saved": (name) => `Saved “${name}” to the keychain`,
  "keys.add": "Add key",
  "keys.field.name": "Name",
  "keys.field.name.example": "e.g. OpenAI",
  "keys.field.env": "Environment variable",
  "keys.field.value": "Value",
  "keys.field.newValue": "New value (leave empty to keep the saved one)",
  "keys.field.paste": "paste the key",
  "keys.field.agents": "Pass it to these agents at startup",
  "keys.save": "Save to keychain",
  "keys.saving": "Saving…",
  "keys.request": "Key requested",
  "keys.request.title": (env) => `A session is asking for the ${env} key`,
  "keys.request.hint": "It stays in the system keychain, and the session gets it the next time it starts. You can also ignore the request.",

  // Video, 3D model, simulator, recording, remote, quota
  "media.pick": "Choose a file",
  "media.loading": "Loading…",
  "model.view.front": "Front",
  "model.view.side": "Side",
  "model.view.top": "Top",
  "model.empty": "Drop a 3D model from the project here, or choose one.",
  "model.triangles": (count) => `${count} triangles`,
  "model.reload": "Reload from disk",
  "model.capture": "Save the view as an image",
  "video.empty": "Drop a video from the project here, or choose one.",
  "video.unreadable": "the file can't be read or its format isn't supported",
  "video.position": "Position",
  "video.speed": "speed",
  "video.speed.label": "Playback speed",
  "video.capture": "Capture this frame",
  "sim.reload": "Reload the app",
  "sim.address": "App address",
  "sim.address.placeholder": "dev server port or URL, e.g. 5173",
  "sim.device": "Device",
  "sim.empty": "Start your app's dev server and open it here.",
  "sim.resize": "Resize the window",
  "sim.resize.tip": "Drag to resize",
  "sim.unreachable": (url) => `No server is responding at ${url}`,
  "sim.unreachable.hint": "Start the dev server (for example, bun run dev) and press Reload.",
  "sim.or": "Or:",
  "record.consent.pane": (pane) => `panel ${pane}`,
  "record.consent.window": "the whole ADE window",
  "record.consent.label": "Recording requested by an agent",
  "record.consent.title": "Record a video?",
  "record.consent.ask": (what) => `An agent wants to record ${what}.`,
  "record.consent.note": "The video captures everything on screen until you stop it with the REC button. Fields with keys and passwords are blurred.",
  "record.consent.mic": "Also record the microphone",
  "record.consent.no": "No",
  "record.consent.yes": "Record",
  "record.export.cantOpen": (source) => `Couldn't open ${source}`,
  "record.export.noCanvas": "The webview doesn't provide a 2D canvas.",
  "record.export.interrupted": "The video stopped during export.",
  "record.chooseFolder": "First choose the folder to save videos in.",
  "record.partial": (problems) => `Video saved, but not every track was: ${problems}`,
  "record.busy": "A recording is already in progress.",
  "record.closingPrevious": "The previous recording is still finishing its file. Try again in a moment.",
  "remote.label": "Add remote environment",
  "remote.title": "Remote environment (ssh)",
  "remote.noSsh": "ssh not found in PATH",
  "remote.target": "Host, user@host or user@host:port",
  "remote.folder": "Remote folder (optional): ~/project or /srv/app",
  "remote.searching": "Looking for hosts in ~/.ssh…",
  "remote.noKnownHost": "No known host matches. Press Enter to connect to what you typed.",
  "remote.noHosts": "No hosts in ~/.ssh/config or known_hosts.",
  "remote.connect": (target) => `Connect to “${target}”`,
  "remote.error.dirStart": "the remote folder must start with / or ~/",
  "remote.error.dirChars": "the remote folder can't contain apostrophes or control characters",
  "remote.error.dirLong": "remote folder path is too long",
  "remote.error.target": "enter host, user@host or user@host:port",
  "remote.error.destination": (destination) => `invalid destination: ${destination}`,
  "remote.error.port": "invalid port",
  "quota.na.tooltip": (vendor, why) => `${vendor} quota: n/a\n${why}`,
  "quota.na.noSource": "No quota source for this agent.",
  "quota.na.noReport": "No quota-axi report found (~/.cache/quota-axi/quotas.json).",
  "quota.na.noWindows": "quota-axi reports no windows for this provider.",
  "quota.na.noTime": "The quota-axi report doesn't say when it was written.",
  "quota.na.future": (time) => `The report is dated ${time}, later than now: it can't be trusted.`,
  "quota.na.claude.noFile": "Claude's quota file wasn't found (~/.llm-quota/official/claude.json): Claude Code's status line writes it.",
  "quota.na.claude.noWindows": "Claude's file reports no quota windows.",
  "quota.na.claude.noTime": "Claude's file doesn't say when it was written.",
  "quota.readClaude": (time) => `Read from Claude Code's status line at ${time}`,
  "quota.staleNote": "Not a recent reading: the quota now may differ.",
  "quota.na.agy.noFile": "agy's quota file wasn't found (~/.llm-quota/official/antigravity.json): agy's status line writes it.",
  "quota.na.agy.noBuckets": "agy's file reports no quota windows.",
  "quota.na.agy.noTime": "agy's file doesn't say when it was written.",
  "quota.readAgy": (time) => `Read from agy's status line at ${time}`,
  "quota.na.agy.future": (time) => `agy's file is dated ${time}, later than now: it can't be trusted.`,
  "quota.readAxi": (time) => `Read from quota-axi at ${time}`,
  "quota.week": "week",
  "quota.tip.title": (name) => `${name} quota`,
  "quota.tip.window": (label, percent) => `${label}: ${percent}% left`,
  "quota.tip.windowResetOn": (label, percent, day) => `${label}: ${percent}% left · resets on ${day}`,
  "quota.tip.windowResetIn": (label, percent, wait) => `${label}: ${percent}% left · resets in ${wait}`,

  // Browser address
  "browser.address.placeholder": "localhost:3000 or port :5173",

  // Notes in panels (what the user reads; the agents get their own Italian text)
  "note.someSession": "a session",
  "note.resent": "Pressed Enter again: the message hadn't been sent",
  "note.rang": (rings) => `Reminder sent again: message not read yet (${rings}/3)`,
  "note.resentRequest": (id) => `Pressed Enter again: request ${id} hadn't been sent`,
  "note.nudged": (id) => `Reminder sent: request ${id} is waiting for a reply`,
  "note.replySent": (ref) => `Reply sent (request ${ref})`,
  "note.replySentTo": (to, ref) => `Reply sent to ${to} (request ${ref})`,
  "note.replyFrom": (from, text) => `Reply from ${from}: ${text}`,
  "note.keptOpen": (problem) => `Kept open: ${problem}`,
  "note.closedAfterReply": (closed) => `Closed after replying: ${closed}`,
  "note.updateFrom": (from, state, text) => `Update from ${from}: ${state} — ${text}`,
  "note.memory": (line) => `Memory: ${line}`,
  "note.subagent": (title) => `Subagent started: ${title}`,
  "note.interruptedBy": (from) => `Interrupted by ${from}`,
  "note.restarted": (from, model) => `Restarted by ${from}${model ? ` with model ${model}` : ""}`,
  "note.restartedFresh": (from, model) => `Restarted from scratch by ${from}${model ? ` with model ${model}` : ""}`,
  "note.askFrom": (from, text) => `Request from ${from}: ${text}`,
  "note.messageFrom": (from, text) => `Message from ${from}: ${text}`,
  "note.askTo": (to, text) => `Request sent to ${to}: ${text}`,
  "note.messageTo": (to, text) => `Message sent to ${to}: ${text}`,
  "voice.nameOnly": (shortcut, manual) =>
    `Starting with this version the assistant starts by voice: begin the sentence with “ei nik” or “nik”. ADE is always listening and ignores sentences without the name; the ${shortcut} shortcut and the button at the top call it by hand. To keep it from listening on its own, choose “${manual}” below.`,
  "voice.shortcutOnly": (shortcut) =>
    `Starting with this version the assistant starts only with the ${shortcut} shortcut or the button at the top, and closes when the answer is done: the microphone no longer stays open, and “ei nik” is gone.`,
  "voice.alwaysListening": (wakeWord, manual, toggle) => `Starting with this version, ADE is always listening and the assistant only answers when you say “${wakeWord}”. To stop it listening on its own, choose “${manual}” below; for an open microphone that answers everything, choose “${toggle}”.`,
  "editor.saveFailed": (problem) => `Save failed: ${problem}`,
  "keys.passed": (names) => `API keys passed: ${names}`,

  // Panel notes found by the extended check
  "media.noBinary": "this host can't read binary files",
  "media.unsupported": (formats) => `unsupported format; supported: ${formats}`,
  "model.missing": (files) => `missing resources: ${files}`,
  "model.captured": (path) => `view saved to ${path}`,
  "preview.noHost": "Files can't be read here.",
  "preview.truncated": "File too large; showing a preview only.",
  "sim.badUrl": "Invalid URL; e.g. 5173, localhost:8081 or http://127.0.0.1:3000",
  "video.unplayable": (formats) => `this format can't be played; supported: ${formats}`,
  "video.captured": (path) => `frame saved to ${path}`,

  // Validation errors: MCP configuration and decision log
  "mcp.error.json": (file) => `${file} doesn't contain valid JSON.`,
  "mcp.error.document": (file) => `${file} must contain a JSON object.`,
  "mcp.error.servers": (file) => `${file}: "mcpServers" must be an object.`,
  "mcp.error.name": "The MCP server name isn't valid.",
  "mcp.error.emptyString": (field) => `${field} must be a non-empty string.`,
  "mcp.error.notObject": "The MCP server configuration isn't an object.",
  "mcp.error.urlOrCommand": "The MCP configuration needs either url or command.",
  "mcp.error.urlAndCommand": "The MCP configuration needs url or command, not both.",
  "mcp.error.remoteType": "A remote MCP server must declare type \"http\" or \"sse\"; without it, Claude Code ignores the server.",
  "mcp.error.stdioType": "An MCP server with a command has type \"stdio\" or no type.",
  "mcp.error.urlCredentials": "url can't contain embedded credentials.",
  "mcp.error.urlSecret": (name) => `url contains a secret value in parameter ${name}; use a reference like "\${VARIABLE_NAME}".`,
  "mcp.error.url": "url must be a valid URL.",
  "mcp.error.args": "args must be an array of strings.",
  "mcp.error.argSecret": (index) => `args[${index}] may contain a secret; use a reference like "\${VARIABLE_NAME}".`,
  "mcp.error.env": "env must be an object of strings.",
  "mcp.error.headers": "headers must be an object of strings.",
  "mcp.error.secretRef": (field) => `${field} must use a reference like "\${VARIABLE_NAME}", not a secret value.`,
  "mcp.error.oauth": "oauth must be an object.",
  "mcp.error.oauthSecret": "oauth.clientId and oauth.clientSecret must use variable references, not secret values.",
  "mcp.error.missing": "The MCP server configuration is missing.",
  "mcp.error.serialize": (file) => `${file} can't be serialized.`,
  "mcp.error.duplicate": (name, file) => `The MCP server "${name}" is already in ${file}.`,
  "mcp.error.tooLarge": (file) => `${file} is too large to rewrite safely.`,
  "mcp.error.read": (file, problem) => `Couldn't read ${file}: ${problem}`,
  "mcp.error.write": (file, problem) => `Couldn't write ${file}: ${problem}`,
  "decisions.log.json": "invalid JSON",
  "decisions.log.invalid": (reason) => `invalid event: ${reason}`,
  "decisions.log.notObject": "not an object",
  "decisions.log.type": "unknown type",
  "decisions.log.key": "missing or invalid key",
  "decisions.log.date": "missing or invalid date",
  "decisions.log.author": "missing author",
  "decisions.log.title": "missing title",
  "decisions.log.order": "order isn't a number",
  "decisions.log.words": "answer without the user's words",
  "decisions.log.until": "deferred without a valid date",
  "decisions.log.options": "options aren't a list",
  "decisions.log.optionLabel": "option without a label",
  "decisions.log.tooLarge": (mb) => `the decision log is larger than ${mb} MB`,
  "decisions.rule.exists": (key) => `${key} already exists`,
  "decisions.rule.neverOpened": (key) => `${key} was never opened`,
  "decisions.rule.closed": (key) => `${key} is already closed`,
  "decisions.rule.answered": (key) => `${key} already has an answer; reopen it first`,
  "decisions.status.aperta": "open",
  "decisions.status.risposta": "answered",
  "decisions.status.rimandata": "deferred",
  "decisions.status.chiusa": "closed",
  "decisions.rule.deferOpen": (key, status) => `only an open decision can be deferred (${key} is ${status})`,
  "decisions.rule.open": (key) => `${key} is already open`,
  "decisions.rule.close": (key) => `${key} can only be closed after an answer or with evidence`,
  "decisions.problem.line": (line, reason) => `line ${line}: ${reason}`,
  "decisions.noProject.short": "no project open",

  // Chat
  "chat.aborted": "Stopped.",
  "chat.error.fallback": "Something went wrong.",
  "chat.model.label": "Model",
  "chat.new": "New conversation",
  "chat.needKey": "You need an OpenRouter key.",
  "chat.openSettings": "Open voice settings",
  "chat.sameAsAssistant": "— it's the same one the assistant uses.",
  "chat.empty.title": "Ask something.",
  "chat.empty.body": "A standard conversation with a model, with no terminal behind it. To have ADE do something, use the Agent section instead.",
  "chat.input.placeholder": "Type a message…",
  "chat.send": "Send",
  "chat.stop": "Stop",
  "chat.writing": "Writing",
  "chat.code.text": "text",
  "chat.code.copied": "Copied",
  "chat.code.copy": "Copy",

  // Bots
  "bots.roster.title": "bots",
  "bots.newBot": "New bot",
  "bots.roster.loading": "Loading…",
  "bots.roster.empty": "No nikcli agents yet. Create one.",
  "bots.noDescription": "No description.",
  "bots.blank.empty": "No bots yet. Press + in the sidebar to create one.",
  "bots.blank.pick": "Choose a bot in the sidebar, or create another one.",
  "bots.face.shape": "Shape",
  "bots.face.color": "Color",
  "bots.face.fromName": "Shape and color come from the name until you pick them.",
  "bots.face.custom": "Your choice.",
  "bots.face.reset": "Reset to name default",
  "bots.subagent.isA": "This is a ",
  "bots.subagent.label": "subagent",
  "bots.subagent.desc": ": it doesn't respond on its own; another agent calls it. You can still message it, but nikcli will use the default agent instead.",
  "bots.permission.request": "Permission request",
  "bots.permission.wantsToUse": "Wants to use",
  "bots.permission.on": "on",
  "bots.permission.reject": "Deny",
  "bots.permission.always": "Always",
  "bots.permission.allow": "Allow",
  "bots.status.replying": "replying…",
  "bots.stop": "Stop",
  "bots.mention.switch": (name) => `Pass the message on to ${name}`,
  "bots.composer.placeholderOthers": (name) => `Message ${name}, or type @ to switch to another bot`,
  "bots.composer.placeholder": (name) => `Message ${name}`,
  "bots.tokens": (count) => `${count} ${count === "1" || count === 1 ? "token" : "tokens"}`,
  "bots.send": "Send",
  "bots.defaultModel": "default model",
  "bots.scope.project": "project",
  "bots.scope.global": "global",
  "bots.terminal.subagentTip": "A subagent doesn't start on its own; another agent calls it",
  "bots.terminal.launchTip": "Opens the nikcli TUI with this bot in a panel",
  "bots.terminal.button": "Terminal",
  "bots.openFile": "Open file",
  "bots.edit.close": "Close",
  "bots.edit.open": "Edit",
  "bots.card.objectives": "Objectives",
  "bots.card.conversation": "Conversation",
  "bots.card.messages": (count) => `${count} ${count === 1 ? "message" : "messages"}`,
  "bots.card.session": (id) => `session ${id}`,
  "bots.card.newTalk": "New conversation",
  "bots.card.file": "File",
  "bots.delete.button": "Delete bot",
  "bots.delete.confirm": (name) => `Delete ${name}?`,
  "bots.delete.cancel": "Cancel",
  "bots.delete.confirmBtn": "Delete",
  "bots.form.problemDesc": "A description is required: nikcli uses it to decide when to call this bot.",
  "bots.form.problemName": "A name is required: it becomes the file identifier.",
  "bots.form.title": "New bot",
  "bots.form.name": "Name",
  "bots.form.namePlaceholder": "Reviewer",
  "bots.form.hintGenerating": "With an empty persona, nikcli chooses the identifier: this name is just a reminder.",
  "bots.form.hintNamed": "Becomes the file name and the identifier passed to --agent.",
  "bots.form.logo": "Avatar",
  "bots.form.description": "Purpose",
  "bots.form.descPlaceholder": "Review pull requests and flag potential risks",
  "bots.form.where": "Where",
  "bots.form.scopeProject": "In project (.nikcli/agent)",
  "bots.form.scopeGlobal": "Global (for all projects)",
  "bots.form.objectives": "Objectives",
  "bots.form.objectivesPlaceholder": "One per line. Kept active across conversations.",
  "bots.form.persona": "Persona",
  "bots.form.personaPlaceholder": "Leave blank and nikcli will write it from the description. Or write your own: how it should behave, and what it should not do.",
  "bots.form.cancel": "Cancel",
  "bots.form.creating": "Creating…",
  "bots.form.generateWithNikcli": "Generate with nikcli",
  "bots.form.create": "Create",
  "bots.form.generatingHint": "nikcli is writing the agent. This takes a few seconds.",
  "bots.engine.label": "Runner",
  "bots.runner.account.nikcli": "Providers connected to nikcli (nikcli auth).",
  "bots.runner.account.claude": "Anthropic subscription or Claude Code API key.",
  "bots.runner.account.codex": "ChatGPT subscription or Codex API key.",
  "bots.engine.accountHint": "Sign-ins are managed in Settings › Providers.",
  "bots.terms.notice": "ADE launches the official CLI installed on your computer, using the account you signed in with, and never sees your credentials. The subscription is for personal use and consumes your plan limits: for intensive or unattended automation use an API key in the CLI.",
  "bots.engine.model": "Model",
  "bots.engine.modelDefaultOf": (label) => `${label} default`,
  "bots.engine.nikcliDefault": "nikcli default",
  "bots.engine.modelsUnavailable": "Model list unavailable: nikcli must be in your PATH. The bot will use the default model.",
  "bots.engine.effort": "Reasoning effort",
  "bots.engine.effortNotSupported": "not supported",
  "bots.engine.effortDefault": "default",
  "bots.settings.whenToUse": "When to use",
  "bots.settings.whenToUseHint1": "The file's",
  "bots.settings.whenToUseHint2": "field: nikcli reads it to decide when to call this bot as a subagent.",
  "bots.settings.objectivesPlaceholder": "One per line.\nE.g. Flag risks before opinions",
  "bots.settings.objectivesHint": "One per line. Added to the prompt under",
  "bots.settings.saved": "Saved.",
  "bots.settings.unsaved": "Unsaved changes.",
  "bots.settings.saving": "Saving…",
  "bots.settings.save": "Save",
}
