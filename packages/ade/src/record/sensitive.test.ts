...
describe("record/sensitive", () => {
  ...
  test("the stylesheet and the selector are the same list, in both directions", () => {
    ...
    const blocks = css
      ...
      // The blurs of D68 (terminal rows, transcript, screenshots) and of a covered pane's viewers are not field covers: they have their own tests.
      .filter((header) => !/\.xterm|pane-transcript|shot-image|:is\(img, video, canvas, svg\)/.test(header))
