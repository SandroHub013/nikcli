...
import { type FileContent, type FileDiff } from "@nikcli-ai/sdk/httpapi"
          ...
          <Accordion multiple value={open()} onChange={handleChange}>
            <For each={props.diffs}>
              {(diff) => {
                ...
                const isAdded = () => diff.before === undefined || (beforeText().length === 0 && afterText().length > 0)
                const isDeleted = () =>
                  diff.after === undefined || (afterText().length === 0 && beforeText().length > 0)
