...
/**
 * The syntax theme every highlighted surface renders with.
 *
 * It lived at module scope in the markdown context, which happened to be
 * imported eagerly — so the registration always ran before anything drew. Once
 * the highlighter became lazy that stopped being true, and the diff and code
 * views started failing with "No valid loader for Nikcli". It belongs next to
 * the highlighter integration, registered by whoever loads it.
 */
const NIKCLI_THEME = {
    name: "Nikcli",
    colors: {
...
/** Idempotent: several lazily loaded entry points race to be first. */
export function registerNikcliTheme() {
  if (registered) return
  registered = true
  registerCustomTheme("Nikcli", () => Promise.resolve(NIKCLI_THEME))
}
