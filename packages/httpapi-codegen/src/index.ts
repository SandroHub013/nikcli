    ...
    const methods = group.endpoints.map((item, endpointIndex) => {
      ...
      const request = (["params", "query", "headers", "payload"] as const)
        .flatMap((source) => {
          ...
          if (fields[0]?.whole) {
            return [
              `${source}: input${item.operation.inputMode === "optional" ? "?." : ""}[${JSON.stringify(fields[0].name)}]`,
            ]
          }
          return [
