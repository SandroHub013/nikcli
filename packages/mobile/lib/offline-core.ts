...
function entry(value: unknown): value is QueueEntry {
  ...
  if (value.type !== "sendMessage" || typeof value.sessionID !== "string" || !value.sessionID) return false
  if (typeof value.text !== "string" || !value.text || typeof value.id !== "string" || !value.id) return false
  if (typeof value.scope !== "string" || !/^[a-f0-9]{64}$/.test(value.scope)) return false
  ...
  return (
    ...
    (model === undefined ||
      (object(model) && typeof model.providerID === "string" && typeof model.modelID === "string"))
  )
}
...
export function createOfflineQueue(adapter: {
  ...
  async function read(): Promise<QueueEntry[]> {
    ...
    try {
      ...
      throw new OfflineQueueError("Offline queue is unreadable. Stored entries were preserved.")
    }
    if (!object(value) || value.version !== 1 || !Array.isArray(value.entries) || !value.entries.every(entry)) {
    ...
    if (new Set(value.entries.map((item) => item.id)).size !== value.entries.length) {
      throw new OfflineQueueError("Offline queue contains duplicate IDs. Stored entries were preserved.")
    }
  ...
  async function context() {
    ...
    if (!value) throw new OfflineQueueError("Connect to the original account and workspace to access queued messages.")
    return value
  }
  return {
    list: () =>
    ...
    enqueue: (op: OfflineOp, state: QueueEntry["state"] = "uncertain") =>
      locked(async () => {
        ...
        if (queue.length >= (adapter.limit ?? 50))
          throw new OfflineQueueError("Offline queue is full. Message was not saved; keep your draft.")
        const next = {
          ...op,
          options: op.options
            ? {
    ...
    drain: (retryID?: string) =>
      locked(async () => {
        ...
        for (const item of queue.slice()) {
          if (item.scope !== current.scope || blocked.has(item.sessionID)) continue
          ...
          queue = queue.map((value) => (value.id === item.id ? { ...value, state: "uncertain" } : value))
          await write(queue)
          try {
