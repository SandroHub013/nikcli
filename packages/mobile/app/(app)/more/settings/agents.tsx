...
export default function AgentsSettingsScreen() {
  ...
  const { data: agents = EMPTY_AGENTS, loading, error, reload } = useHostResource<AgentInfo[]>(fetcher, [client])
  ...
  useEffect(() => {
    ...
    const isMissing = /Request failed with 404/.test(error) || error.toLowerCase().includes("not found")
    if (isMissing) {
