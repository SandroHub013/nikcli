    ...
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      <div class="sticky top-0 z-10 bg-[linear-gradient(to_bottom,var(--surface-raised-stronger-non-alpha)_calc(100%_-_24px),transparent)]">
        <div class="flex flex-col gap-1 pt-6 pb-8 max-w-[720px]">
          ...
          <p class="text-14-regular text-text-base">{language.t("settings.commands.description")}</p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-8 w-full max-w-[720px]">
        <Show
          when={directory()}
          fallback={<p class="text-14-regular text-text-base">{language.t("settings.worktrees.empty.project")}</p>}
        >
          <Show
            when={items().length > 0}
            fallback={<p class="text-14-regular text-text-base">{language.t("settings.commands.empty")}</p>}
          >
