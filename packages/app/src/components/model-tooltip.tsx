...
export const ModelTooltip: Component<{ model: ModelInfo; latest?: boolean; free?: boolean }> = (props) => {
  ...
  return (
    <div class="flex flex-col gap-1 py-1">
      ...
      <Show when={inputs()}>
        {(value) => (
          <div class="text-13-regular text-text-invert-base">
            {language.t("model.tooltip.allows", { inputs: value() })}
          </div>
        )}
      </Show>
      <div class="text-13-regular text-text-invert-base">{reasoning()}</div>
      <div class="text-13-regular text-text-invert-base">{context()}</div>
    </div>
  )
}
