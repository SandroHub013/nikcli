...
import { useCodeComponent } from "@nikcli-ai/ui/context/code"
import { createAutoScroll } from "@nikcli-ai/ui/hooks"
...
import { checksum, base64Encode } from "@nikcli-ai/util/encode"
...
export default function Page() {
  ...
  const kinds = createMemo(() => {
    ...
    for (const diff of diffs()) {
      ...
      const beforeEmpty = diff.before.length === 0
      const afterEmpty = diff.after.length === 0
      const kind = beforeEmpty && !afterEmpty ? "add" : afterEmpty && !beforeEmpty ? "del" : "mix"
    ...
    <div data-component="session-shell" class="relative bg-background-base size-full overflow-hidden flex flex-col">
      ...
      <div data-slot="session-workspace" class="flex-1 min-h-0 flex flex-col md:flex-row">
        ...
        >
          ...
          <Show when={desktopReviewOpen() && supportsInSessionResize()}>
            <ResizeHandle
