...
export const log = Log.create({ service: "stats-command" })
...
export default Runtime.handler(Commands.commands["stats"], async (input) => {
  ...
  await bootstrap(process.cwd(), async (instance) => {
    ...
    // `--models` was untyped in yargs, so a bare flag arrived as `true`. Effect
    // declares it a string, so "all models" is now any non-numeric value.
    if (args.models !== undefined && !Number.isFinite(Number(args.models))) {
      modelLimit = Infinity
