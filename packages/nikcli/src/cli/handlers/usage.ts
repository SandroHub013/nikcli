...
export const log = Log.create({ service: "usage-command" })

export function runSession<A, E>(effect: Effect.Effect<A, E, Session.Service>): Promise<A> {
...
export function displayUsage(stats: UsageStats, sessions: SessionUsage[], args: DisplayArgs): void {
  const WIDTH = 68
...
export function renderMiniBar(percentage: number, width: number): string {
...
export async function getAllSessions(): Promise<z.infer<typeof SessionSchema>[]> {
