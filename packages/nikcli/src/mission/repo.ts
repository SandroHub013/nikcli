...
export namespace MissionRepo {
  ...
  export function upsert(projectId: string, def: MissionDefinition, executor?: Executor) {
    return Database.query(
      "MissionRepo.upsert",
      (db) => {
  ...
  export function putExec(projectId: string, exec: MissionExec, executor?: Executor) {
    return Database.query(
      "MissionRepo.putExec",
      (db) => {
        const row = toExecRow(projectId, exec)
        db.insert(missionExec)
          .values(row)
          .onConflictDoUpdate({
  ...
  /** Newest first. */
  export function listExecs(projectId: string, missionId: string, limit?: number, executor?: Executor) {
    return Database.query(
      "MissionRepo.listExecs",
      (db) => {
        const query = db
          .select({ data: missionExec.data })
          .from(missionExec)
  ...
  export function listExecsByStatus(projectId: string, status: MissionExec["status"], executor?: Executor) {
    return Database.query(
      "MissionRepo.listExecsByStatus",
      (db) => {
        const rows = db
          .select({ data: missionExec.data })
          .from(missionExec)
          .where(and(eq(missionExec.projectId, projectId), eq(missionExec.status, status)))
          .orderBy(desc(missionExec.startedAt))
          .all()
        return defined(rows.map((row) => readExec(row.data)))
      },
   ...
   */
  export function trimExecs(projectId: string, missionId: string, limit: number) {
    return Database.transaction((tx) =>
      Effect.sync(() => {
        const ids = tx
  ...
  export function countExecRecords(projectId: string, missionId: string, executor?: Executor) {
    return Database.query(
      "MissionRepo.countExecRecords",
      (db) => {
        const row = db
