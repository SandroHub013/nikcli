...
export namespace MissionRepo {
  ...
  export function get(projectId: string, id: string): MissionDefinition | undefined {
    const row = db()
      .select({ data: mission.data })
      .from(mission)
  ...
  export function upsert(projectId: string, def: MissionDefinition, executor: Executor = db()): void {
  ...
  /** Newest first. */
  export function listExecs(projectId: string, missionId: string, limit?: number): MissionExec[] {
    const query = db()
      .select({ data: missionExec.data })
      .from(missionExec)
  ...
  export function listExecsByStatus(projectId: string, status: MissionExec["status"]): MissionExec[] {
    const rows = db()
      .select({ data: missionExec.data })
      .from(missionExec)
