...
describe("onboarding account step", () => {
  test("requires the shared web OAuth flow before onboarding can continue", async () => {
    ...
    expect(appSource).toMatch(
      /runOnboarding:\s*\(\)\s*=>\s*DialogOnboarding\.run\(dialog\)/,
    );
    ...
    expect(appSource).not.toMatch(
      /import\(["']@tui\/component\/dialog-(onboarding|login|account-login)["']\)/,
    );
  });
  ...
  test("never records onboarding as complete without an account", async () => {
    ...
    expect(before.lastIndexOf('outcome.status === "complete"')).toBeGreaterThan(
      before.lastIndexOf("ensureOnboarded"),
    );
  });
});
