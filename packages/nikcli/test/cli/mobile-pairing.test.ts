...
import {
  ...
  shouldShowPairingLink,
} from "@tui/component/dialog-mobile-connect";
import {
  asciiQRRuns,
  qrCanImageFit,
  qrDialogBudget,
...
describe("mobile pairing", () => {
  ...
  test("normalizes cloud server and mobile endpoint URLs", () => {
    expect(normalizeMobileServerUrl("cloud.example.com/")).toBe(
      "https://cloud.example.com",
    );
    expect(
      normalizeMobileServerUrl(
        "https://cloud.example.com/base/mobile/teleport",
      ),
    ).toBe("https://cloud.example.com/base");
  ...
  test("fits an overflowing pairing QR into the remaining dialog cells", () => {
    const matrix = Array.from({ length: 53 }, () =>
      Array.from({ length: 53 }, () => true),
    );
  ...
  test("CLI terminal QR on Windows is 16-color spaces, not █▀▄", async () => {
    const url =
      "nikcli://connect?server=http://192.168.1.4:4096&token=nkm_secret";
