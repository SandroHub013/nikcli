...
describe("Workspace HttpApi bridge", () => {
  it("serves workspace adaptor, create, restore, session restore, list, and remove routes", async () => {
    ...
    const session = (await post("/session", directory, {
    ...
    const detached = (await request(`/session/${session.id}`, directory)) as {
      workspaceID?: string
    }
    ...
    const warped = (await request(`/session/${session.id}`, directory)) as {
      workspaceID?: string
    }
    ...
    await Bun.sleep(2)
    await post("/session", directory, {
      title: "Workspace usage session",
      workspaceID,
    })
