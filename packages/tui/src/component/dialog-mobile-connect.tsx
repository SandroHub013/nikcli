...
function isPlainShortcut(
  evt: { ctrl?: boolean; meta?: boolean; super?: boolean; name?: string },
  ...names: string[]
) {
 ...
 */
export function shouldShowPairingLink(
    ...
    <Show when={shouldShowPairingLink()}>
      <box marginTop={1} flexDirection="column">
        <text fg={theme.foreground.muted} wrapMode="word">
          If the QR is blank, this terminal could not draw it. Press y to copy
          the same pairing link and open it on the phone:
        </text>
...
export function DialogMobileConnect(props: {
  sessionID?: string;
  initialMode?: MobileDialogMode;
}) {
  const [mode, setMode] = createSignal<MobileDialogMode>(
    props.initialMode ?? "choose",
  );
  ...
  const options = createMemo<DialogSelectOption<MobileDialogMode>[]>(() => [
    {
      ...
      value: "cloud",
      description:
        "Enter a remote nikcli server URL and mobile auth token, then scan its QR code.",
    },
    {
      title: "Connect local server",
      value: "local",
      description:
        "Start a protected LAN server and create a mobile pairing token automatically.",
    },
    {
...
function RemoteServerPanel(props: {
  mode: "cloud" | "teleport";
  sessionID?: string;
  onBack: () => void;
}) {
  ...
  const qrRows = createMemo(() =>
    pairing() ? qrRenderHeight(pairing()!.matrix) : 0,
  );
  const qrWidth = createMemo(() =>
    pairing() ? qrRenderWidth(pairing()!.matrix) : 0,
  );
  ...
  onMount(() => {
    ...
    const saved = (sync.data.config as RemoteServerConfig | undefined)
      ?.teleport;
  ...
  async function saveRemoteServer(url: string, token: string) {
    await sdk.client.config
      .update({ payload: { teleport: { url, token } } as any })
      .catch(() => undefined);
  }
  ...
  async function teleport() {
    ...
    try {
      ...
      if (info.directory) {
        ...
        const archive = await createWorkspaceArchive(info.directory).catch(
          () => null,
        );
        if (archive) {
      ...
      setForm(
        "status",
        `Teleporting ${messages.length} messages${uploadID ? " + workspace" : ""}…`,
      );
      const response = await fetch(`${base}/mobile/teleport`, {
        ...
        body: JSON.stringify({
          title: info.title,
          name: info.directory
            ? info.directory.split(/[\\/]/).filter(Boolean).pop()
            : undefined,
      ...
      setForm(
        "status",
        cause instanceof Error ? cause.message : "Failed to teleport session",
      );
    } finally {
      setForm("busy", false);
    }
  ...
  async function copyDeepLink() {
    ...
    await Clipboard.copy(value)
      .then(() =>
        toast.show({ message: "Mobile link copied", variant: "success" }),
      )
      .catch(toast.error);
  }
          ...
          <>
            ...
            <box flexDirection="row" gap={1} alignItems="center">
              ...
              <box
                ...
                border={["bottom"]}
                borderColor={
                  form.focus === "url" ? theme.accent.fg : theme.border.subtle
                }
            ...
            <box flexDirection="row" gap={1} alignItems="center">
              ...
              <box
                ...
                border={["bottom"]}
                borderColor={
                  form.focus === "token" ? theme.accent.fg : theme.border.subtle
                }
            ...
            <Show when={form.status}>
              <text
                fg={
                  form.busy ? theme.foreground.muted : theme.status.warning.fg
                }
                wrapMode="word"
              >
                {form.status}
            ...
            >
              <box
                ...
                paddingRight={1}
                backgroundColor={
                  form.busy ? theme.surface.offset : theme.accent.fg
                }
          ...
          <>
            ...
            >
              <box flexDirection="column" gap={1} flexGrow={1} minWidth={32}>
                ...
                <text fg={theme.foreground.muted} wrapMode="word">
                  This QR connects the app directly to the verified cloud
                  server.
                </text>
                <text fg={theme.foreground.muted}>Server URL</text>
                <text fg={theme.accent.alt} selectable wrapMode="word">
...
function LocalMobileConnect(props: { onBack: () => void }) {
  ...
  async function resolveBaseUrl() {
    ...
    const started = await sdk.client.mobile.host.lan.start(
      { mdns: true },
      { throwOnError: true },
    );
    ...
    if (!url)
      throw new Error(
        "The host did not open a LAN listener for mobile pairing",
      );
    return url;
  }
  ...
  async function createPairing() {
    ...
    try {
      ...
      if (urls.length === 0) {
        throw new Error(
          "No LAN address found. Connect both devices to the same network or use a public server URL.",
        );
      }
        ...
        {(value) => (
          ...
          >
            <box flexDirection="column" gap={1} flexGrow={1} minWidth={32}>
              ...
              <text fg={theme.foreground.muted} wrapMode="word">
                The QR securely pairs this server and workspace. It enables
                mobile sessions, approvals, remote actions and Teleport
                workflows.
              </text>
              <box marginTop={1} flexDirection="column">
                <text fg={theme.foreground.muted}>Server URL</text>
              ...
              <Show when={value().urls.length > 1}>
                <text fg={theme.foreground.muted}>
                  tab switches network interface ({selectedURL() + 1}/
                  {value().urls.length})
                </text>
              </Show>
              <Show when={isLikelyIPhoneHotspotUrl(value().serverUrl)}>
                <text fg={theme.status.warning.fg} wrapMode="word">
                  iPhone Personal Hotspot may block access from the phone to
                  connected devices. Use the same Wi-Fi network, Tailscale, or
                  the cloud server if this connection times out.
                </text>
              </Show>
              <box marginTop={1} flexDirection="column">
                <text fg={theme.foreground.muted}>Pairing token</text>
