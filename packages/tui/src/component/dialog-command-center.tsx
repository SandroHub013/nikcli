...
import {
  buildCommandCenterSpec,
  latticeFromSync,
  type CommandCenterSource,
} from "@tui/util/command-center";
...
export function DialogCommandCenter() {
  ...
  const layout = createMemo(() =>
    computeAnalyticsDialogLayout(dimensions().width, dimensions().height),
  );
  ...
  async function load() {
    ...
    try {
      ...
      void analyticsCtx.refreshSessions().then(() => {
        if (liveBase)
          setStats(withHistory(liveBase, source() === "live+history"));
      });
    } catch (e) {
      setLoadError(
        e instanceof Error ? e.message : "Command center could not be loaded",
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          ...
          >
            ...
            >
              <Renderer
                spec={spec()!}
                loading={analyticsCtx.loading() && source() !== "live+history"}
              />
            </box>
          </scrollbox>
        </box>
